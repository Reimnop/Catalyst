...
public class GameDataLevelObjectsConverter
{
    ...
    private Sequence<Engine.Math.Vector2> GetVector2Sequence(GameData.EventKeyframes eventKeyframes, Vector2Keyframe defaultKeyframe, bool relative = false)
    {
        System.Collections.Generic.List<IKeyframe<Engine.Math.Vector2>> keyframes = new System.Collections.Generic.List<IKeyframe<Engine.Math.Vector2>>(eventKeyframes.keyframes.Count);

        Engine.Math.Vector2 currentValue = Engine.Math.Vector2.Zero;
        foreach (EventKeyframe eventKeyframe in eventKeyframes.keyframes)
        {
            Engine.Math.Vector2 value = new Engine.Math.Vector2(eventKeyframe.Vals[0], eventKeyframe.Vals[1]);
